...
func (this *Request) Send(a ...interface{}) *Request {
	...
	if len(a) == 0 || a[0] == "url" {
		var body io.Reader
		if this.method != "GET" {
			body = strings.NewReader(this.data.Encode())
		}
		this.request, err = http.NewRequest(this.method, this.url, body)
		...
		} else if this.method == "GET" && len(this.data) > 0 {
			this.request.URL.RawQuery = this.data.Encode()
		}
	} else if a[0] == "json" {
